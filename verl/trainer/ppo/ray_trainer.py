...
def apply_length_penalty(
    ...
    length_start = max_response_length / 2.0
    ...
    for i in range(len(data)):
        ...
        if response_length <= length_start:
            continue
        ...
        if response_length >= max_response_length:
            ...
            weight = (
                 (response_length - length_start)
                / (max_response_length - length_start)
            )

        penalty = length_penalty_coef * weight

        reward_tensor[i, response_length - 1] -= penalty
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            for k in test_batch.non_tensor_batch.keys():
                if 'success_rate' in k or 'draw_rate' in k or 'success&draw_rate' in k or 'score' in k:
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        if self.config.actor_rollout_ref.actor.get('use_length_penalty', False):
                            print('use length penalty')
                            batch = apply_length_penalty(batch,
                                                                        max_response_length=self.config.data.max_response_length,
                                                                        length_penalty_coef=self.config.actor_rollout_ref.actor.length_penalty_coef)
