...
class TrajectoryCollector:
    ...
    def vanilla_multi_turn_loop(
        ...
        for attempt_idx, phase, steps in phase_and_steps:
            ...
            if self.config.get('use_trial_norm', False):
                obs['anchor'] = [f'Trial {attempt_idx}: {anchor}' for anchor in obs['anchor']]
                print('Use trial norm! anchor:', obs['anchor'])
        ...
        if self.config.env.get('multipolicy', False):
            print('multipolicy success evaluate')
            success: Dict[str, np.ndarray] = envs.success_evaluator_opp(
    ...
    def credit_assignment(self, total_batch_list, num_attempts, step_gamma=0.95, traj_gamma=0.6):
        ...
        print(f"total_batch_list len: {len(total_batch_list)} should be equal to batch_size*group_size: {self.config.data.train_batch_size}*{self.config.env.rollout.n}")
        for steps in total_batch_list:
            ...
            diff_cumulative_reward = [cumulative_reward[0]] + list(np.diff(cumulative_reward))
            # calculate discounted returns
            running_return = 0
            discounted_returns = [0.] * len(steps)
            curr_traj_idx = steps[-1]['traj_idx']
            diff_step_rewards = []
            for traj_idx, group in groupby(steps, key=lambda x: x['traj_idx']):
                group_list = list(group)
                for i, step in enumerate(group_list):
                    if i == len(group_list) - 1:
                        diff_step_rewards.append(diff_cumulative_reward[traj_idx])
                    else:
                        diff_step_rewards.append(0.0)
            # print(f"diff step rewards: {diff_step_rewards}")  
            for t in reversed(range(len(steps))):
                step = steps[t]
                if step['phase'] == 'play':
                    traj_idx = step['traj_idx']
                    if traj_idx == curr_traj_idx:
                        running_return = diff_step_rewards[t] + step_gamma * running_return
                    else:
                        running_return = diff_step_rewards[t] + traj_gamma * running_return
            ...
            for t in range(len(steps)):
                step = steps[t]
                if step['phase'] == 'play':
                    episode_rewards.append(diff_cumulative_reward[step['traj_idx']])
                else:
                    episode_rewards.append(0.)
            # print(f"cumulative reward: {cumulative_reward}, diff_cumulative_reward: {diff_cumulative_reward}, episode rewards: {episode_rewards}")
